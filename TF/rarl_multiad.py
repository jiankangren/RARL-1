...
class RARL(TRPO):
    ...
    def init_opt(self):
        ...
        # extra_dims=1 + is_recurrent
        # name = 'obs'+'_2_'+str(i)
        # obs_var1 = tf.placeholder(tf.float32, shape=[None] * extra_dims + [self.obs2_dim], name=name)

        # name = 'action'+'_2_'+str(i)
        # action_var1 = tf.placeholder(tf.float32, shape=[None] * extra_dims + [self.action2_dim], name=name)

        # advantage_var1 = tensor_utils.new_tensor(
        #     'advantage'+'_2_'+str(i),
        #     ndim=1 + is_recurrent,
        #     dtype=tf.float32,
        # )
        ...
        # old_dist_info_vars1 = {
        #     k: tf.placeholder(tf.float32, shape=[None] * (1 + is_recurrent) + list(shape), name='old_%s' % k+'_2_'+str(i))
        #     for k, shape in dist.dist_info_specs
        #     }
        # old_dist_info_vars_list = [old_dist_info_vars1[k] for k in dist.dist_info_keys]

        # state_info_vars1 = {
        #     k: tf.placeholder(tf.float32, shape=[None] * (1 + is_recurrent) + list(shape), name=k+'_2_'+str(i))
        #     for k, shape in self.policies2[i].state_info_specs
        #     }
        # state_info_vars_list = [state_info_vars1[k] for k in self.policies2[i].state_info_keys]

        # if is_recurrent:
        #     valid_var1 = tf.placeholder(tf.float32, shape=[None, None], name="valid"+'_2_'+str(i))
        # else:
        #     valid_var1 = None

        # dist_info_vars1 = self.policies2[i].dist_info_sym(obs_var1, state_info_vars1)
        # kl1 = dist.kl_sym(old_dist_info_vars1, dist_info_vars1)
        # lr1 = dist.likelihood_ratio_sym(action_var1, old_dist_info_vars1, dist_info_vars1)
        # if is_recurrent:
        #     mean_kl1 = tf.reduce_sum(kl1 * valid_var1) / tf.reduce_sum(valid_var1)
        #     surr_loss1 = - tf.reduce_sum(lr1 * advantage_var1 * valid_var1) / tf.reduce_sum(valid_var1)
        # else:
        #     mean_kl1 = tf.reduce_mean(kl1)
        #     surr_loss1 = - tf.reduce_mean(lr1 * advantage_var1)
        ...
        #              ] + state_info_vars_list + old_dist_info_vars_list
        # if is_recurrent:
        #     input_list.append(valid_var1)

        # self.optimizers2[i].update_opt(
        #     loss=surr_loss1,
        #     target=self.policies2[i],
        #     leq_constraint=(mean_kl1, self.step_size),
        #     inputs=input_list,
        #     constraint_name="mean_kl"
        # )
        ...
        #     if is_recurrent:
        #         valid_var = tf.placeholder(tf.float32, shape=[None, None], name="valid"+'_2_'+str(i))
        #     else:
        #         valid_var = None
