...
from TF.GIM_sampler import RARLSampler
...
class RARL(TRPO):
    ...
    def obtain_samples(self, itr, policy_num):
        return self.sampler.obtain_samples(itr, policy_num, self.action2_limit)
    @overrides
        ...
        for itr in range(self.start_itr, self.n_itr):
            ...
            for n1 in range(self.N1):
                with logger.prefix('itr #%d ' % itr + 'n1 #%d |' % n1):
                    ...
                    paths = self.obtain_samples(itr=itr,policy_num=1)
                ...
                for n2 in range(self.N2):
                    if itr != self.n_itr-1: #don't train adversary at last time
                        with logger.prefix('itr #%d ' % itr + 'n2 #%d |' % n2):
                            ...
                            logger.log("Obtaining samples...")
                            paths = self.obtain_samples(itr=itr,policy_num=2)
                            logger.log("Processing samples...")
