...
def _worker_populate_task(G, env, policy1, policies2, scope=None):
    G = _get_scoped_G(G, scope)
    G.env = pickle.loads(env)
    G.policy1 = pickle.loads(policy1)
    G.policies2 = pickle.loads(policies2)
...
def _worker_terminate_task(G, scope=None):
    ...
    if getattr(G, "policy2", None):
        for i in range(len(G.policies2)):
            G.policies2[i].terminate()
            G.policies2[i] = None

def populate_task(env, policy1, policies2, scope=None):
    logger.log("Populating workers...")
    if singleton_pool.n_parallel > 1:
        singleton_pool.run_each(
            _worker_populate_task,
            [(pickle.dumps(env), pickle.dumps(policy1), pickle.dumps(policies2), scope)] * singleton_pool.n_parallel
        )
    else:
...
def _worker_set_policy_params(G, params1, params2, policy2_num,scope=None):
    G = _get_scoped_G(G, scope)
    G.policy1.set_param_values(params1)
    G.policies2[policy2_num].set_param_values(params2)
...
def sample_paths(
    ...
    singleton_pool.run_each(
        ...
        [(policy1_params, policy2_params, policy2_num,scope)] * singleton_pool.n_parallel
    )
    ...
    return singleton_pool.run_collect(
        ...
        args=(policy_num, policy2_num,max_path_length, scope),
        show_prog_bar=True
    )
